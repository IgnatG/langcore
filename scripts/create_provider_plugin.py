"""Create a new LangCore provider plugin with all boilerplate code.
...
https://github.com/ignatg/langcore/blob/main/langcore/providers/README.md
...
def create_directory_structure(package_name: str, force: bool = False) -> Path:
    ...
    base_dir = Path(f"langcore-{package_name}")
    package_dir = base_dir / f"langcore_{package_name}"
...
def create_pyproject_toml(
    ...
    content = textwrap.dedent(
        ...
        {package_name} = "langcore_{package_name}.provider:{provider_name}LanguageModel"

        [tool.setuptools.packages.find]
        where = ["."]
        include = ["langcore_{package_name}*"]
    """
    )
...
def create_provider(
    ...
    schema_imports = (
        f"""
from langcore_{package_name}.schema import {provider_name}Schema"""
        if with_schema
        else ""
    )
    ...
    init_content = textwrap.dedent(
        f'''\
        """LangCore provider plugin for {provider_name}."""

        from langcore_{package_name}.provider import {provider_name}LanguageModel
...
def create_readme(
    ...
    readme_content = textwrap.dedent(
        f"""\
        # LangCore {provider_name} Provider

A provider plugin for LangCore that supports {provider_name} models.
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Create a new LangCore provider plugin",
...
def print_summary(
    ...
    print("https://github.com/ignatg/langcore/blob/main/langcore/providers/README.md")
