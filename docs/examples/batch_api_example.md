...
The Vertex AI Batch API offers significant cost savings (~50%) for large, non-time-critical workloads. `langcore` seamlessly integrates this with automatic routing, caching, and fault tolerance.
...
# 4. Run Extraction
# langcore will automatically chunk the text and submit a batch job.
results = lx.extract(
...
`langcore-{project}-{location}-batch`
...
LangCore's batch processing is designed to minimize costs:
...
### 1. Automatic Routing
`langcore` automatically switches between real-time and batch APIs based on your `threshold`.
...
### 3. Automated Storage
`langcore` handles all GCS operations automatically using a dedicated bucket (`gs://langcore-{project}-{location}-batch`). Note that input/output files are retained for debugging.
...
When running a batch job, `langcore` provides clear log feedback with a direct link to the Google Cloud Console:
...
- **Completion**: Once the job succeeds, `langcore` automatically downloads, parses, and aligns the results.
