...
from langcore import chunking, progress, prompting
...
def _merge_non_overlapping_extractions(
    ...
    each merged extraction by combining:

    - **Cross-pass frequency**: ``appearances / total_passes``
    - **Alignment confidence**: the per-extraction score assigned
      during ``Resolver.align()`` (based on alignment quality and
      token overlap ratio)

    The final score is ``cross_pass * alignment_confidence``, keeping
    it in ``[0.0, 1.0]``.

    For large extraction counts (>= ``_INTERVAL_TREE_THRESHOLD``), an
    ``IntervalTree`` is used for O(log n) overlap queries instead of the naive
    O(n**2) pairwise scan.
    ...
    Returns:
      ...
      ``confidence_score`` is set to ``cross_pass * alignment_confidence``.
    """
    ...
    # For multi-pass, combine cross-pass frequency with the
    # per-extraction alignment confidence that was already set during
    # the alignment phase (``Resolver.align``).
    if total_passes > 1:
        for ext, count in zip(merged_extractions, appearance_counts):
            cross_pass = count / total_passes
            alignment_conf = ext.confidence_score  # set by resolver
            if alignment_conf is not None:
                ext.confidence_score = round(cross_pass * alignment_conf, 4)
            else:
                ext.confidence_score = round(cross_pass, 4)
...
class Annotator:
    ...
    def __init__(
        ...
        self._hooks = hooks or hooks_lib.Hooks()
    ...
    def _annotate_documents_single_pass(
        ...
        **kwargs,
        ...
        per_doc: collections.defaultdict[str, list[data.Extraction]] = (
        ...
        try:
            for batch in batch_iter:
                ...
                # Emit LLM call hook with prompts and outputs.
                self._hooks.emit(
                    hooks_lib.HookName.EXTRACTION_LLM_CALL,
                ...
                for text_chunk, scored_outputs in zip(batch, outputs):
                    ...
                    # Emit alignment hook for each chunk.
                    self._hooks.emit(
                        hooks_lib.HookName.EXTRACTION_ALIGNMENT,
                        {
                    ...
                    # Emit chunk hook after processing.
                    self._hooks.emit(
                        hooks_lib.HookName.EXTRACTION_CHUNK,
                        {
                            "chunk_text": text_chunk.chunk_text,
                            "document_id": text_chunk.document_id,
                            "extractions": aligned_extractions,
    ...
    async def _async_annotate_documents_single_pass(
        ...
        **kwargs,
        ...
        doc_usage: collections.defaultdict[str, dict[str, int]] = (
