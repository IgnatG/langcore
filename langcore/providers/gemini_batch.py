"""Gemini Batch API helper module for LangCore.
...
def _get_bucket_name(project: str | None, location: str) -> str:
    ...
    base = f"langcore-{project}-{location}-batch".lower()
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            for key_data, text in items:
                ...
                if not isinstance(text, str):
                    try:
                        text = json.dumps(
                            text, default=_json_default, ensure_ascii=False
                        )
                    except Exception as e:
...
def infer_batch(
    ...
    display_base = f"langcore-batch-{int(time.time())}"
