...
def load_plugins_once() -> None:
    ...
    Discovers and loads langcore provider plugins using entry points.
    ...
    if os.environ.get("LANGCORE_DISABLE_PLUGINS", "").lower() in (
        "1",
        "true",
        "yes",
    ):
        logging.info("Plugin loading disabled via LANGCORE_DISABLE_PLUGINS")
    ...
    try:
        ...
        # Python 3.12+ API
        provider_eps = eps.select(group="langcore.providers")
