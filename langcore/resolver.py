...
# when computing per-extraction confidence.
_W_ALIGNMENT_DEFAULT: Final[float] = 0.7
_W_OVERLAP_DEFAULT: Final[float] = 0.3
...
class ResolverParsingError(exceptions.LangCoreError):
...
class Resolver(AbstractResolver):
    ...
    def align(
        ...
        **kwargs,
        ...
        if not extractions:
            logging.debug(
                "No extractions found in the annotated text; exiting alignment process."
            )
            return
        else:
        ...
        for extraction in itertools.chain(*aligned_yaml_extractions):
            extraction.confidence_score = compute_alignment_confidence(
                extraction, tokenizer_inst=tokenizer_inst
            )
...
def compute_alignment_confidence(
    ...
    *,
    ...
    2. **Token overlap ratio** — the number of tokens in the extraction
       ...
       the alignment-quality score so it acts as an identity.

    The final score is the weighted average:

    .. math::

        \\text{confidence} = w_{\\text{alignment}} \\times \\text{alignment\\_quality}
                           + w_{\\text{overlap}} \\times \\text{overlap\\_ratio}

    Parameters:
      extraction: An ``Extraction`` that has already been aligned (i.e.
        ``alignment_status`` and ``token_interval`` may have been set).
      tokenizer_inst: Optional tokenizer for counting extraction tokens.
      w_alignment: Weight for alignment quality (default 0.7).  Must
    ...
    if extraction.token_interval is not None and extraction.extraction_text:
        extraction_tokens = list(
            _tokenize_with_lowercase(
                extraction.extraction_text,
                tokenizer_inst=tokenizer_inst,
            )
        )
        span_size = (
            extraction.token_interval.end_index - extraction.token_interval.start_index
        )
        if span_size > 0 and extraction_tokens:
            overlap_ratio = min(len(extraction_tokens) / span_size, 1.0)

    return round(
        wa * alignment_quality + wo * overlap_ratio,
        4,
    )
