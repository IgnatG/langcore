...
def _create_mock_entry_points(entry_points_list):
    ...
    Args:
      entry_points_list: List of entry points to return for langcore.providers.
...
class PluginSmokeTest(absltest.TestCase):
    ...
    def test_plugin_disabled_by_env_var(self):
        """Test that LANGCORE_DISABLE_PLUGINS=1 prevents plugin loading."""

        with mock.patch.dict("os.environ", {"LANGCORE_DISABLE_PLUGINS": "1"}):
...
class PluginE2ETest(absltest.TestCase):
    ...
    def test_pip_install_discovery_and_cleanup(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            pkg_name = f"test_langcore_plugin_{uuid.uuid4().hex[:8]}"
        ...
        from langcore.core import types
      ...
      """)
        ...
        name = "{pkg_name}"
        version = "0.0.1"
        description = "Test plugin for langcore"
