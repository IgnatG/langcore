...
from langcore import exceptions, factory
from langcore.core import base_model, types
...
class FactoryTest(absltest.TestCase):  # pylint: disable=too-many-public-methods
    def setUp(self):
        super().setUp()
        router.clear()
        import langcore.providers as providers_module  # pylint: disable=import-outside-toplevel
    ...
    @mock.patch.dict(os.environ, {"LANGCORE_API_KEY": "env-langcore-key"}, clear=True)
    def test_falls_back_to_langcore_api_key_when_provider_key_missing(self):
        """Factory uses LANGCORE_API_KEY when provider-specific key is missing."""
        config = factory.ModelConfig(model_id="gemini-pro")

        model = factory.create_model(config)
        self.assertEqual(model.api_key, "env-langcore-key")

    @mock.patch.dict(
        os.environ,
        {
            "GEMINI_API_KEY": "gemini-key",
            "LANGCORE_API_KEY": "langcore-key",
        },
    )
    def test_provider_specific_key_takes_priority_over_langcore_key(self):
        """Factory prefers provider-specific API key over LANGCORE_API_KEY."""
        config = factory.ModelConfig(model_id="gemini-pro")
