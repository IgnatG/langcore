...
from langcore.providers import gemini_batch as gb
...
GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY") or os.environ.get("LANGCORE_API_KEY")
...
skip_if_no_gemini = pytest.mark.skipif(
    not GEMINI_API_KEY,
    reason=("Gemini API key not available (set GEMINI_API_KEY or LANGCORE_API_KEY)"),
)
...
class TestLiveAPIGemini(unittest.TestCase):
    ...
    def test_multilingual_medication_extraction(self):
        ...
        assert len(medication_extractions) > 0, (
