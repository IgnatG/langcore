"""LangCore benchmark suite for performance and quality testing.
...
import langcore
import langcore.io as lio
from benchmarks import config, plotting, utils
from langcore import core, data, visualize
from langcore.core import exceptions as langcore_exceptions

# Load API key from environment
dotenv.load_dotenv(override=True)
GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY", os.environ.get("LANGCORE_API_KEY"))
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    break
                except langcore_exceptions.InferenceRuntimeError as inf_err:
                    err_str = str(inf_err)
            ...
            RuntimeError,
            langcore_exceptions.InferenceRuntimeError,
        ) as e:
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="LangCore Benchmark Suite")
    ...
    parser.add_argument(
        "--english-only",
        action="store_true",
        help=("Only test English text type (avoids rate limits on free-tier API keys)"),
    )
    ...
    if args.compare:
        ...
        comparison_path = (
            results_dir / f"comparison_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
        )
    ...
    if "gemini" in model_to_test.lower() and not GEMINI_API_KEY:
        print(f"Error: {model_to_test} requires GEMINI_API_KEY or LANGCORE_API_KEY")
        return
    if _is_openai_model(model_to_test) and not OPENAI_API_KEY:
        print(f"Error: {model_to_test} requires OPENAI_API_KEY in environment or .env")
        return
